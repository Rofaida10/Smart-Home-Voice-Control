"""Audio In Out and feature extraction.
...
class AudioLoader:
    ...
    leading and trailing silence and normalizes volume
    ...
    def load(self, filepath: str) -> tuple[np.ndarray, int]:
        ...
        # Peak normalize so loudness differences between recordings ormics don't become an accidental "feature"
...
class FeatureExtractor:
    ...
    def extract(self, signal: np.ndarray, sr: int) -> np.ndarray:
        ...
        # Average over time so every clip to one fixed length vector regardless of how long the recording was.
...
# Module-level singleton + shortcut function so anyone can just do: from sounds import extract_features
